...
#define TST_KEY_LIFETIME 15 // FIXME 4096
...
static tst_ctx_t * tst_key_create(const char *secret, size_t secret_len, uv_loop_t *loop)
{
	...
	tst_ctx_t *key = malloc(offsetof(tst_ctx_t, hash_data) + hash_len);
	if (!key) return NULL;
	key->hash_len = hash_len;
	if (secret_len) {
		memcpy(key->hash_data + sizeof(time_t), secret, secret_len);
	}
...
static int tst_key_update(tst_ctx_t *key, time_t epoch, bool force_update)
{
	if (!key || key->hash_len < sizeof(epoch)) {
		assert(!EINVAL);
		return kr_error(EINVAL);
	}
	if (!force_update && memcmp(key->hash_data, &epoch, sizeof(epoch)) == 0) {
		return kr_ok(); /* we are up to date */
		/* TODO: support mixing endians? */
	}
	memcpy(key->hash_data, &epoch, sizeof(epoch));

	if (key->hash_len > sizeof(epoch)) {
		#if !TLS_SESSION_RESUMPTION_SYNC
			assert(false);
			return kr_error(ENOTSUP);
		#endif
		/* Deterministic variant of the rotation. */
		int err = gnutls_hash_fast(TST_HASH, key->hash_data,
...
static void tst_key_destroy(uv_handle_t *timer)
{
	...
	assert(key);
	gnutls_memset(key, 0, offsetof(tst_ctx_t, hash_data) + key->hash_len);
	free(key);
}
...
static void tst_key_check(uv_timer_t *timer, bool force_update)
{
	...
	const time_t epoch = (now.tv_sec + stst->epoch_shift) / TST_KEY_LIFETIME;
...
{
	assert(session);
	if (!ctx) return; /* no tickets desired */
